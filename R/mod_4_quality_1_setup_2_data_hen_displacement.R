...
mod_4_quality_1_setup_2_data_hen_displacement_ui <- function(id) {
  ...
  tagList(
    ...
    shiny::selectInput(
      inputId = ns("cock_val"),
      label = "Code for cocks / broilers",
      choices = NULL,
      selected = NULL
    ),
    shiny::selectInput(
      inputId = ns("hen_val"),
      label = "Code for hens / layers",
      choices = NULL,
      selected = NULL
    ),
    shiny::selectInput(
      inputId = ns("pullet_val"),
      label = "Code for pullets / DOCs",
      choices = NULL,
      selected = NULL
    ),
    shiny::actionButton(
...
mod_4_quality_1_setup_2_data_hen_displacement_server <- function(id, r6, parent){
  moduleServer(id, function(input, output, session){
    ...
    input_choices <- shiny::reactiveValues(
      data = r6$hen_displacement_df_choices,
    ...
    if (!is.null(r6$cow_displacement_provided)) {
      ...
      shiny::updateSelectInput(
        inputId = "data",
        choice = r6$hen_displacement_df_choices,
