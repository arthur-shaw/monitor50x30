...
mod_1_setup_2_suso_creds_server <- function(id, parent, r6) {
  moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(input$save, {
      ...
      # ensure that all inputs provided
      iv <- shinyvalidate::InputValidator$new()
      iv$add_rule("server", shinyvalidate::sv_required())
      iv$add_rule("workspace", shinyvalidate::sv_required())
      ...
      all_creds_provided <- shiny::reactive({
        all(
          shinyvalidate::input_provided("server"),
          shinyvalidate::input_provided("workspace"),
          shinyvalidate::input_provided("user"),
          shinyvalidate::input_provided("password"),
          input$server    != "",
          input$workspace != "",
          input$user      != "",
          input$password  != ""
